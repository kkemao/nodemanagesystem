$(function() {
  ...
  $IF.tips = function(flag, parentClassName, time, poi, jump) {
    if (document.getElementsByClassName("animateTips001").length != 0) {
      document
        .getElementsByClassName(parentClassName)[0]
        .removeChild(document.getElementsByClassName("animateTips001")[0]);
    }
    var poi = poi || "center";
    var jump = jump || "";
...
//密码效验
window.validPassword = {
  register($dom, type) {
    let tip = "至少6位，需要同时包含数字、大小写字母、符号(@$!%*)";
    if (type === 1) {
      $dom.text("密码" + tip);
    } else {
      $dom.attr("placeholder", tip);
    }
  },
  valid($inputDom, type) {
    let tip = "";
    let pattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{6,}/;
    if (pattern.test($inputDom.val())) {
      if (type === 1) {
      } else {
        $inputDom.next(".error span").text("");
      }
      return true;
    } else {
      if (type === 1) {
