...
(function ($) {
  ...
  watermark.prototype = {
    ...
    init: function (options) {
      ...
      var fixH =
        settings.maxWidth * Math.abs (Math.sin (settings.deg)) +
        Math.cos (settings.deg) * settings.textHeight;
      if (fixH > settings.height) settings.height = fixH;
      var ctx1 = this.__setCanvasStyle (
        can1,
        settings.canvasWidth,
        settings.height
      );
      var ctx2 = this.__setCanvasStyle (
        can2,
        settings.canvasWidth,
        settings.height
      );
      var ctx = this.__setCanvasStyle (
        canAll,
        settings.canvasWidth,
        settings.height * 2,
        true
    ...
    __calcTextSize: function ($container) {
      var txts = [], maxWidth = 0, canvasWidth = 0, settings = this.options;
      $.each (settings.texts, function (i, text) {
        ...
        var tWidth = span[0].offsetWidth, tHeight = span[0].offsetHeight;
        span.remove ();
        txts.push ({
          txt: text,
          width: tWidth,
        ...
        canvasWidth +=
          (settings.width < shadow ? shadow : settings.width) -
          tHeight * Math.sin (settings.deg);
      });
      settings.txts = txts;
    ...
    __setCanvasStyle: function (canvas, width, height, notextstyle) {
      ...
      if (!notextstyle) {
        var deg = this.options.deg, absSindeg = Math.abs (Math.sin (deg));
        ctx.rotate (deg);
        //基于视窗的 x、y偏移量
        var offset =
          absSindeg * this.options.height - this.options.textHeight * absSindeg;
        var nx = -offset * Math.cos (deg), ny = -offset * absSindeg;
